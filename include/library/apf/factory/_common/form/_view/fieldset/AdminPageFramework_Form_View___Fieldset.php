...
abstract class TaskScheduler_AdminPageFramework_Form_View___Fieldset_Base extends TaskScheduler_AdminPageFramework_Form_Utility {
    ...
    private function _loadScripts($sStructureType = '') {
        if ('widget' === $sStructureType && !self::$_bIsLoadedJSScripts_Widget) {
            new TaskScheduler_AdminPageFramework_Form_View___Script_Widget;
        ...
        new TaskScheduler_AdminPageFramework_Form_View___Script_Utility;
        new TaskScheduler_AdminPageFramework_Form_View___Script_OptionStorage;
        new TaskScheduler_AdminPageFramework_Form_View___Script_AttributeUpdator;
        new TaskScheduler_AdminPageFramework_Form_View___Script_RepeatableField($this->oMsg);
        new TaskScheduler_AdminPageFramework_Form_View___Script_SortableField;
    }
...
jQuery( document ).ready( function() {
    var _oButtonPlaceHolders = jQuery( '#{$sFieldsContainerID} > .task-scheduler-field.without-child-fields .repeatable-field-buttons' );
    ...
    /* Otherwise, insert the button element at the beginning of the field tag */
         ...
         */
        if ( ! jQuery( '#{$sFieldsContainerID} .task-scheduler-repeatable-field-buttons' ).length ) { 
            jQuery( '#{$sFieldsContainerID} > .task-scheduler-field.without-nested-fields' ).prepend( $_sSmallButtons );
        }
        /**
         * Support for nested fields.
         * For nested fields, add the buttons to the fields tag.
         */
        jQuery( '#{$sFieldsContainerID} > .task-scheduler-field.with-nested-fields' ).prepend( $_sNestedFieldsButtons );
        
        /**
         * Support for inline mixed fields.
         */
        // jQuery( '#{$sFieldsContainerID} > .task-scheduler-field.with-mixed-fields' ).prepend( $_sNestedFieldsButtons );
        
    }     
    jQuery( '#{$sFieldsContainerID}' ).updateTaskScheduler_AdminPageFrameworkRepeatableFields( $_aJSArray ); // Update the fields     
});
JAVASCRIPTS;
    ...
    private function ___getRepeatableButtonHTML($sFieldsContainerID, array $aArguments, $iFieldCount, $bSmall = true) {
        $_oFormatter = new TaskScheduler_AdminPageFramework_Form_Model___Format_RepeatableField($aArguments, $this->oMsg);
    ...
    protected function _getSortableFieldEnablerScript($sFieldsContainerID) {
        $_sScript = <<<JAVASCRIPTS
    jQuery( document ).ready( function() {
        jQuery( this ).enableTaskScheduler_AdminPageFrameworkSortableFields( '$sFieldsContainerID' );
    });
JAVASCRIPTS;
        return "<script type='text/javascript' class='task-scheduler-sortable-field-enabler-script'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
    }
